...
Supported agent targets are Claude Code, Cursor, Codex, OpenCode, and Pi — pass the target with the `-a` flag, for example `npx skills add steel-dev/skills --skill steel-browser -a cursor -g`. Beyond these built-in targets, Steel Skills generally work with any agent that supports the agent skills format.
:::

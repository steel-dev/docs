...
:::callout
Profiles preserve browser identity. [Dedicated IPs](/overview/sessions-api/dedicated-ips) preserve network identity. Using both can improve login stickiness because sites see the same cookies, storage, and a familiar IP instead of a fresh browser from a new network every run.
:::

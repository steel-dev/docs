...
The default session timeout is 5 minutes. You can extend it with the `timeout` option on session create (for example `timeout: 1800000` for 30 minutes) and set `inactivityTimeout` to release after a period of inactivity.

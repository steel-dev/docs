...
*   The returned `session.websocketUrl` to attach Playwright or Puppeteer. It contains a temporary JWT for the session, so no API key needs to be appended. See [authentication](/overview/authentication#temporary-session-jwts).
...
5 minutes — after that the session is automatically released. You can change this by passing the `timeout` parameter (in milliseconds) when creating the session, for example `timeout: 600000` for 10 minutes.

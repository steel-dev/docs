---
title: Build a LangGraph Browser Agent on Steel
sidebarTitle: LangGraph
description: Build a LangGraph browser agent on Steel cloud browsers. Model web tasks as explicit state machines with nodes, edges, and structured output in Python.
llm: true
---
...
Inside a `@tool`, call `steel.sessions.create()` and connect Playwright with `chromium.connect_over_cdp(session.websocket_url)`, then use `browser.contexts[0].pages[0]` as the shared page for your other tools.

### Does LangGraph work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — enable them when the tool creates the session (for example `use_proxy`, `solve_captcha`, `stealth_config` on `sessions.create()`). The graph and `ToolNode` wiring are unaffected.

...
<Link href="https://playground.steel.dev/?template=steel-agent-kit-starter"><Button><img src="https://api.scalar.com/cdn/images/jBw8j7D0nDuWr2AD2vuO5/fPY1vc0gHOyKJLz6lRIZ8.png" alt="Try in Playground" width="20"/> Try in Playground</Button></Link>
        ...
        const items = await page.evaluate((maxItems: number) => {
          ...
          for (let i = 0; i < take; i++) {
            ...
            const commentsNum = /\d+/.test(commentsText)
              ? parseInt((commentsText.match(/\d+/) || ["0"])
[0], 10)
              : 0;
...
*   Session Lifecycle: [https://docs.steel.dev/overview/sessions-api/session-lifecycle](https://docs.steel.dev/overview/sessions-api/session-lifecycle)

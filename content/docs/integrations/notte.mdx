...
Create a session with `client.sessions.create()`, build `cdp_url = session.websocket_url`, and open `notte.Session(cdp_url=cdp_url)` as a context manager, passing it to `notte.Agent(session=notte_session)`.

### Does Notte work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — enable them on `sessions.create()` (for example `use_proxy`, `solve_captcha`, `stealth_config`). Notte connects over the CDP URL and is unaware of how the session was provisioned.

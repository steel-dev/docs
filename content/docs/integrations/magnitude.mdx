...
const agent = await startBrowserAgent({
  ...
  browser: { cdp: session.websocketUrl },
});
```
...
Create a session with `client.sessions.create()`, then set `browser.cdp` in the `startBrowserAgent` config to the session's `websocketUrl` directly.

### Does Magnitude work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — configure them when creating the session (for example `useProxy`, `solveCaptcha`, `stealthConfig` on `sessions.create()`). Magnitude just connects to the resulting CDP endpoint.

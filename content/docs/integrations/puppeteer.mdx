---
title: Connect Puppeteer to Steel Cloud Browsers (CDP)
sidebarTitle: Puppeteer
description: Run Puppeteer in the cloud with Steel. Connect over CDP in TypeScript with no local Chrome, plus built-in stealth, rotating proxies, and CAPTCHA solving.
llm: true
---
...
Create a session with `client.sessions.create()`, then call `puppeteer.connect()` with `browserWSEndpoint` set to the session's `websocketUrl` directly and open a tab with `browser.newPage()`.
:::

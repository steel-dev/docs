---
title: Run Browser Use in the Cloud on Steel
sidebarTitle: Browser Use
description: Run Browser Use in the cloud with Steel. Point the open-source Python agent at a Steel session and let LLMs navigate, fill forms, and extract data at scale.
llm: true
---
...
Create a session with `client.sessions.create()`, build the CDP URL as `session.websocket_url`, and pass it to `BrowserSession(cdp_url=cdp_url)` on the `Agent`.

### Does Browser Use work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — these are session-creation options (for example `use_proxy`, `solve_captcha`, `stealth_config` on `sessions.create`), invisible to Browser Use itself. The page links dedicated recipes for auto-solving CAPTCHAs and handing them to a human.

...
Call `steel.sessions.create()`, connect with `chromium.connectOverCDP()` using the session's `websocketUrl` directly, and take the first page of `browser.contexts()[0]` as the page your AgentKit tool handlers drive.

### Does AgentKit work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — pass them to `sessions.create()` (for example `useProxy`, `solveCaptcha`, `stealthConfig`). They're Steel-side session options, so your agent network and routers don't change.
:::

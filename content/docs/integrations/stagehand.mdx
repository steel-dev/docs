---
title: Run Stagehand on Steel Cloud Browsers
sidebarTitle: Stagehand
description: Run Stagehand on Steel cloud browsers. Write automations in natural language with act, extract, and observe. Stealth and proxies are managed for you.
llm: true
---
...
Create a session with `client.sessions.create()`, then set `cdpUrl` inside `localBrowserLaunchOptions` to the session's `websocketUrl` directly, and call `stagehand.init()`.

### Does Stagehand work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — set them when creating the Steel session (for example `useProxy`, `solveCaptcha`, `stealthConfig` on `sessions.create`). Stagehand connects over CDP and is unaware of how the session was provisioned.
:::

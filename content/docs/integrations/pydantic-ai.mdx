---
title: Build a Pydantic AI Browser Agent on Steel
sidebarTitle: Pydantic AI
description: "Build a Pydantic AI browser agent on Steel cloud browsers: provider-agnostic, typed Python agents with dependency injection and real web access."
llm: true
---
...
Create a session with `steel.sessions.create()`, connect via `chromium.connect_over_cdp(session.websocket_url)`, take `browser.contexts[0].pages[0]`, and pass it as `deps=BrowserDeps(page=page)` to `agent.run()`.

### Does Pydantic AI work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — set them on `sessions.create()` (for example `use_proxy`, `solve_captcha`, `stealth_config`). The agent only sees a Playwright `Page` through `ctx.deps`, so session options never leak into tool code.
:::

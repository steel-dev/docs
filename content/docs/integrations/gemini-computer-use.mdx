...
Yes — the page notes it pairs well with Steel's anti-bot capabilities, proxy support, and sandboxed environments. These are session-creation options (for example `useProxy`, `solveCaptcha`, `stealthConfig`) and don't change the agent loop.

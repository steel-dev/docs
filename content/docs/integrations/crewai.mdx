---
title: Build a CrewAI Browser Agent on Steel
sidebarTitle: CrewAI
description: Give CrewAI agents a real browser with Steel. Orchestrate multi-agent Python crews that navigate the web, with shared memory and human-in-the-loop reviews.
llm: true
---
...
Yes — when your tool creates Steel sessions you can enable these at session creation (for example `use_proxy`, `solve_captcha`, `stealth_config`). They're Steel-side options, so CrewAI's orchestration doesn't change.

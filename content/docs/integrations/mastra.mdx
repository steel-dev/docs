...
Define a `createTool` whose `execute` calls `steel.sessions.create()` and connects via `chromium.connectOverCDP()` with the session's `websocketUrl` directly, register it on an `Agent` via `tools`, and attach the agent to a top-level `Mastra` registry.

### Does Mastra work with Steel's proxies, stealth mode, and CAPTCHA solving?

Yes — pass the options to `sessions.create()` inside your tool (for example `useProxy`, `solveCaptcha`, `stealthConfig`). Mastra's tool layer is unaffected because these are Steel session settings.
:::

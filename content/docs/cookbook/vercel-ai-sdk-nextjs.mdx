...
<RecipeGrid>
<RecipeCard slug="google-adk" title={"Build a browser agent with Google ADK"} description={"Use Steel with Google's Agent Development Kit (ADK) for Go to build a tool-using browser agent that drives a chromedp session over CDP and reads Hacker News."} topics={['Agents', 'Typed output']} date="2026-06-23" />

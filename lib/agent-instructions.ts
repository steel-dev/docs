...
import { docsPath, docsUrl } from './docs-path';
...
- Brand design language (DESIGN.md): ${docsUrl('/DESIGN.md')}
...
- Any docs page is available as markdown by appending \`.md\` to its URL, for example \`${docsUrl('/overview/steel-cli.md')}\`; AI user agents (Claude, Cursor, GPT) receive markdown automatically at the canonical URL
- \`${docsPath('/llms-full.txt')}\` concatenates every page into one file (large, roughly 230k tokens); prefer fetching the individual \`.md\` pages you need over reading the whole bundle

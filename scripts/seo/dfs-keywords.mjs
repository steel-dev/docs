...
const positional = args.filter((a, i) => !a.startsWith('--') && i !== outFlag + 1 && i !== fileFlag + 1);

let keywords = [];
if (stdinMode) keywords.push(...readFileSync(0, 'utf8').split('\n').map(s => s.trim()).filter(Boolean));
if (file) keywords.push(...readFileSync(file, 'utf8').split('\n').map(s => s.trim()).filter(Boolean));
...
async function dfs(path, payload) {
  ...
  if (json.status_code === 40104) {
    throw new Error('DataForSEO account is not verified (40104). Verify at https://app.dataforseo.com/ then re-run.');
  }
...
async function keywordInfoBatch(kws) {
  ...
  return rows.map((item) => ({
    ...
    trend: (item.monthly_searches ?? []).map(m => `${m.year}-${String(m.month).padStart(2,'0')}:${m.search_volume}`),
...
async function serp(kw) {
  ...
  return {
    ...
    results: items.map(it => ({
      rank: it.rank_group, type: it.type, domain: it.domain,
      title: it.title, url: it.url, description: it.description,
    })),
  };
}
...
if (serpMode) {
  for (const kw of keywords) {
    try {
      ...
      if (String(e.message).includes('40104')) { console.error(`\n${e.message}`); process.exit(3); }
